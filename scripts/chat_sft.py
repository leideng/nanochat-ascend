...
wandb_run = DummyWandb() if use_dummy_wandb else wandb.init(project="nanochat-ascend-sft", name=args.run, config=user_config)
...
identity_conversations_filepath = get_global_config().identity_conversations_dataset
...
def sft_data_generator_bos_bestfit(split, buffer_size=100):
    ...
    while True:
        ...
        # Build tensors
        use_npu = device_type == "npu"
        batch_tensor = torch.tensor(rows, dtype=torch.long, pin_memory=use_npu)
        inputs = batch_tensor[:, :-1].to(device=device, dtype=torch.int32, non_blocking=use_npu)
        targets = batch_tensor[:, 1:].to(device=device, dtype=torch.int64, non_blocking=use_npu)
...
while True:
    ...
    if master_process and last_step and not args.dry_run:
        ...
        checkpoint_dir = os.path.join(get_global_config().chatsft_checkpoints_dir, output_dirname)
        save_checkpoint(
            checkpoint_dir,
            step,
    ...
    for micro_step in range(grad_accum_steps):
        ...
        loss.backward()
        x, y = next(train_loader) # prefetch the next batch while the NPU is busy with forward/backward
