...
from nanochat.common import compute_init, compute_cleanup, print0, DummyWandb, print_banner, get_global_config, autodetect_device_type, get_peak_flops
...
while True:
    ...
    for micro_step in range(grad_accum_steps):
        ...
        loss.backward()
        x, y, dataloader_state_dict = next(train_loader) # prefetch the next batch while the NPU is busy with forward/backward
    # step the optimizer
    ...
    train_loss_f = train_loss.item() # .item() is a CPU-NPU sync point
    synchronize()
    t1 = time.time()
    dt = t1 - t0
    # -------------------------------------------------------------------------
